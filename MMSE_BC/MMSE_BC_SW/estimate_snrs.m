function [temp, a_post_snr,a_priori_snr]=estimate_snrs(noisy_dft_frame_p,fft_size, noise_psd ,SNR_LOW_LIM,  ALPHA  ,I,clean_est_dft_frame_p)  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%a posteriori SNR
a_post_snr = noisy_dft_frame_p./(noise_psd(1:fft_size/2+1)); 
temp = 0;
if I==1
    ...
    %       =========================
   a_priori_snr = max( ALPHA*( clean_est_dft_frame_p(1:fft_size/2+1)) ./...
       ( noise_psd(1:fft_size/2+1)) + (1-ALPHA)*(a_post_snr-1), SNR_LOW_LIM );
   temp = max( ALPHA*( sum(clean_est_dft_frame_p(1:fft_size/2+1)))./...
   ( sum(noise_psd(1:fft_size/2+1))) + (1-ALPHA)*(a_post_snr-1), SNR_LOW_LIM );
end
